...
function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (datos) {
      ...
      const movemetsArr = datos.account.movements.map(obj => { return Number(obj.amount) })
        ...
        <form className="col-md-6  mt-3 m-0">
          ...
          <div className="d-flex justify-content-end p-3 bg-warning ms-auto m-0">
            <div className="mb-3 mx-3 ">
              ...
              <input
                type="text"
                className="form-control input-user" id="userId"
                value={user}
                onChange={(e) => setUser(e.target.value)}
              />
              <div id="emailHelp" className="form-text">usa tus credenciales</div>
            </div>
            <div className="mb-3">
            ...
            <div className="d-flex align-items-center ms-2 btn-login">
              <p type="submit" className="h-25 " onClick={handleSubmit}><i className="fa fa-arrow-right" aria-hidden="true"></i></p>
      ...
      {logued &&
        <div className='row' >
          ...
          <div className=" col-md-6 operaciones">
            <Operar data={data} saldo={saldo} token={token} user={user} />
          </div>
